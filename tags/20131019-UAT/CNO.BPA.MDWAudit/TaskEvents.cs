...
namespace CNO.BPA.MDWAudit
{
    public class TaskEvents : ITaskEvents
    {
                        ...
                        case "STANDARD_MDF":
                            {
                                ...
                                //START:AR122352
                                taskInfo.Task.Batch.Tree.Values(wfStep).SetString("GENERATE_XLS", BatchDetail.GenerateXLS);
                                taskInfo.Task.Batch.Tree.Values(wfStep).SetString("DEFAULT_XLS_PATH", BatchDetail.DefaultXLSPath);
                                setDBCredentials(taskInfo, parmsCustom);
                                //END:AR122352
        ...
        //START:AR122352
        private void setDBCredentials(ITaskInformation taskInfo, CustomParameters parmsCustom)
        {
            try
            {
                ...
                foreach (IWorkflowStep wfStep in taskInfo.Task.Batch.WorkflowSteps)
                {
                    if (wfStep.Name.ToUpper() == "STANDARD_MDF")
                    {
                        taskInfo.Task.Batch.Tree.Values(wfStep).SetString("DB_USER", parmsCustom.UserName);
                        taskInfo.Task.Batch.Tree.Values(wfStep).SetString("DB_PASS", parmsCustom.Password);
                        taskInfo.Task.Batch.Tree.Values(wfStep).SetString("DSN", parmsCustom.DSN);
                    }
                }
