    ...
    public class MDWAuditModule : CustomCodeModule
    {
        ...
        private void processMDWMappings(IClientTask task, IBatchContext batchContext)
        {
            try
            {
                ...
                //changed XMLvalue read step from 3 to 0. [21.4 Upgrade]
                 foreach (IBatchNode env in currentNode.GetDescendantNodes(3))
               // foreach (IBatchNode env in currentNode.GetDescendantNodes(0))
                {
                    ...
                    if (!object.ReferenceEquals(BatchDetail.mdwMappings, null))
                    {
                        ...
                        {
                            foreach (IBatchNode lvl in currentNode.GetDescendantNodes(0))
                            {


                                string contentValue = string.Empty;

                                IBatchNode importStepNode = batchContext.GetStepNode(lvl, "IMPORT");
                ...
                switch (BatchDetail.InputSource)
                {
                    case "FAX":
                        #region CASE "FAX"
                        {
                            ...
                            BatchDetail.PrepDate = importStepNode.NodeData.ValueSet.ReadString("DateModified", "");
        ...
        private void    setBatchValues(IClientTask task, IBatchContext batchContext)
        {
            try
            {
                ...
                try
                {
                    ...
                    autoClassifyP7.NodeData.ValueSet.WriteString("IADepartments", BatchDetail.Department);
                ...
                try
                {
                    ...
                    autoValidationP7.NodeData.ValueSet.WriteString("IADepartments", BatchDetail.Department);
                ...
                try
                {
                    ...
                    manualClassifyP7.NodeData.ValueSet.WriteString("IADepartments", BatchDetail.Department);
                ...
                try
                {
                    ...
                    //manualValP7.NodeData.ValueSet.WriteString("IADepartments", BatchDetail.Department);
                ...
                try
                {
                    //IBatchNode manualIndexP7 = batchContext.GetRoot("ManualIndex");
                    //manualIndexP7.NodeData.ValueSet.WriteString("IADepartments", BatchDetail.Department);
                    IBatchNode stepNode = batchContext.GetStepNode(task.BatchNode, "ManualIndex");
                    foreach (IBatchNode item in (IEnumerable<IBatchNode>)stepNode.GetDescendantNodes(3))
                    {
                        item.NodeData.ValueSet.WriteString("IATaskRouting", BatchDetail.Department);
                    }
                }
                catch (Exception ex)
                {
        ...
        private void getEnvelopeDetails(IClientTask task, IBatchContext batchContext)
        {
            try
            {
                ...
                //else
                //{
                IBatchNodeCollection envelopes = node.GetDescendantNodes(2);
                //}
                        ...
                        case "EMAIL":
                            {
                                ...
                                //IBatchNodeCollection envelopes = node.GetDescendantNodes(2);
                                //TODO: Test
                                IBatchNode emailEnvNode = batchContext.GetStepNode(env, "IMPORT");
                                ...
                                BatchDetail.PrepDate = emailEnvNode.NodeData.ValueSet.ReadString("EmailDate", "");
                                BatchDetail.ReceivedDateCRD = BatchDetail.PrepDate;

                                envelopeDetail[i].EmailSender = emailEnvNode.NodeData.ValueSet.ReadString("EmailFromAddress", "");
                                envelopeDetail[i].EmailTo = emailEnvNode.NodeData.ValueSet.ReadString("EmailTo", "");
        ...
        private void setEnvelopeValues(IClientTask task, IBatchContext batchContext)
        {
            try
            {
                ...
                foreach (IBatchNode node in currentNode.GetDescendantNodes(2))
                {
