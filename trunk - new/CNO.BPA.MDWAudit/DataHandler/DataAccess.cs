...
namespace CNO.BPA.MDWAudit.DataHandler
{
   public class DataAccess : IDisposable
   {
      ...
      public string getBatchNo()
      {
       ...
       //NICK
      public string getBatchNo(string date)
      {
          try
          {
              string batchNo = string.Empty;
              ...
              DBUtilities.CreateAndAddParameter("p_in_received_date",
                Convert.ToDateTime(date).ToString("yyMMdd"), OracleType.VarChar, ParameterDirection.Input, cmd);
              DBUtilities.CreateAndAddParameter("p_out_batch_no",
                OracleType.VarChar, ParameterDirection.Output, 15, cmd);
              DBUtilities.CreateAndAddParameter("p_out_result",
                 OracleType.VarChar, ParameterDirection.Output, 255, cmd);
       ...
       //NICK
      public Dictionary<string, string> getMDWMappings()
      {
          try
          {
              Dictionary<string, string> mdwMappings = new Dictionary<string, string>();
              Connect();
              OracleCommand cmd = GenerateCommand("bpa_apps.pkg_ia.select_ia_mdw_definition", CommandType.StoredProcedure);
              ...
              DBUtilities.CreateAndAddParameter("p_in_input_source",
                BatchDetail.InputSource, OracleType.VarChar, ParameterDirection.Input, cmd);
              ...
              using (OracleDataReader dataReader = cmd.ExecuteReader())
              {
                  ...
                  else
                  {
                      if (dataReader.HasRows)
                      {
                          ...
                          {
                              mdwMappings.Add(dataReader.GetString(2), dataReader.GetString(3));
                          }
                          Disconnect();
                          return mdwMappings;
                      }
          ...
          {
              throw new Exception("DataHandler.DataAccess.getMDWMappings: " + ex.Message);
          }
      }
      ...
      {          
          try
          {
              ...
              DBUtilities.CreateAndAddParameter("p_in_faxid",
                 DocumentDetail.FaxID, OracleType.VarChar, ParameterDirection.Input, cmd);
              DBUtilities.CreateAndAddParameter("p_in_fax_key",
                 DocumentDetail.FaxKey, OracleType.VarChar, ParameterDirection.Input, cmd);              
      ...
      public void updateImportXref()
      {
          try
          {
              Connect();
              OracleCommand cmd = GenerateCommand("bpa_apps.create_ia_import_xref", CommandType.StoredProcedure);
              ...
              DBUtilities.CreateAndAddParameter("p_in_app_name",
                 "CNO.BPA.MDWAUDIT", OracleType.VarChar, ParameterDirection.Input, cmd);
              DBUtilities.CreateAndAddParameter("p_in_machine_name",
                 System.Environment.MachineName.ToUpper(), OracleType.VarChar, ParameterDirection.Input, cmd);
              DBUtilities.CreateAndAddParameter("p_in_user_id",
                 System.Environment.UserName.ToUpper(), OracleType.VarChar, ParameterDirection.Input, cmd);     
              DBUtilities.CreateAndAddParameter("p_in_guid",
                 DocumentDetail.dGUID, OracleType.VarChar, ParameterDirection.Input, cmd);
          ...
          {
              throw new Exception("DataHandler.DataAccess.updateImportXref: " + ex.Message);
          }
      }
